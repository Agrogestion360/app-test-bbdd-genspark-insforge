...
const INSFORGE_URL = (globalThis as any).INSFORGE_URL || 'https://insforge.agrogestionx.com'
const INSFORGE_KEY = (globalThis as any).INSFORGE_KEY || 'ik_48c0eac742486d9c15b318e67ecec74689168ba0ff194451fffc930f1d2f5016'

const app = new Hono()
app.use('*', cors())

// ── Helper BD (requiere admin key) ───────────────────────────
async function db(path: string, method = 'GET', body?: object, userToken?: string) {
  const headers: Record<string, string> = {
    'Authorization': `Bearer ${userToken || INSFORGE_KEY}`,
...
// ── Helper Auth API ──────────────────────────────────────────
async function authApi(path: string, method: string, body?: object, token?: string) {
...
// ── Decodificar JWT (sin verificar firma, solo leer payload) ─
...
async function requireAuth(c: any, next: any) {
  ...
  // Decodificar JWT localmente (InsForge no tiene endpoint /me)
  const payload = decodeJwt(token)
  if (!payload) {
    deleteCookie(c, 'session_token', { path: '/' })
    return c.redirect('/login')
  }
  // El JWT de InsForge tiene: sub (id), email, role — sin name
  // Usamos el email como display name por defecto
...
// POST /api/auth/signup → Registro via admin API de InsForge
...
// GET /api/auth/me → Info del usuario actual (decodifica JWT local)
...
// ── API TAREAS (protegidas) ──────────────────────────────────

app.get('/api/tareas', requireAuth, async (c) => {
  const semana = c.req.query('semana')
  const path = semana
    ? `/tareas?semana=eq.${semana}&order=created_at.desc`
    : `/tareas?order=created_at.desc`
